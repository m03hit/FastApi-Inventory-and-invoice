...
def create_purchase(purchase: purchase.PurchaseCreate, db: Session = Depends(get_db)):
    ...
    added_Purchase = models.Purchase(date=purchase.date, title=purchase.title, amount=purchase.amount,
                                     supplier_id=purchase.supplier_id)
    db.add(added_Purchase)
    db.commit()
    db.refresh(added_Purchase)
    for expense in base_purchase.purchase_expenses:
        db.add(models.PurchaseExpense(amount=expense.amount, title=expense.title, description=expense.description,
                                      purchase_id=added_Purchase.id))
        db.commit()

    for product_item in base_purchase.products:
        db.add(models.ProductItem(unit_price=product_item.unit_price,
                                  effective_unit_price=product_item.effective_unit_price,
                                  date_purchased=product_item.date_purchased, quantity=product_item.quantity,
                                  product_id=product_item.product_id, purchase_id=added_Purchase.id,
