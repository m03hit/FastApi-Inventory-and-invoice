...
def create_invoice(db:Session,invoice:invoice.InvoiceCreate):
    db_invoice = models.Invoice(amount=invoice.amount,date=invoice.date,profit=invoice.profit,customer_id=invoice.customer_id)
    db.add(db_invoice)
    db.commit()
...
def create_invoice_items(db: Session,invoice:invoice.InvoiceCreate,invoice_id_int:int):
    ...
    for item in invoice.invoice_items:
        db.add(models.InvoiceItem(amount=item.amount,unit_price=item.unit_price,quantity=item.quantity,product_id=item.product_id,invoice_id=invoice_id_int))
